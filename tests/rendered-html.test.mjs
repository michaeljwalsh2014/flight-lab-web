...
test("adds a conversational, evidence-aware zero-cost Flight Lab Coach", async () => {
  ...
  assert.match(coach, /v40 response mode/);
  assert.match(coach, /type="range"/);
  assert.match(coach, /THINKING_MODE_OPTIONS/);
  ...
  assert.match(coach, /selectedModel === "v40" \? null/);
  ...
  assert.doesNotMatch(coach, /Response mode|Always look it up|Never search/);
  assert.match(coach, /const searchMode = coachSearchMode\(clean\)/);
  assert.match(coach, /Every v40 reply is generated by Gemini/);
  assert.match(coach, /aria-pressed/);
  assert.match(coach, /Searching with Gemini/);
