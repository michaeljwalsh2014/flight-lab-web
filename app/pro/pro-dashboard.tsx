  ...
  return <section className="pro-tool-section" id="plane-coach">
    ...
    <div className="pro-plane-grid">
      <div className="pro-upload-panel">
        <label>AI version<select aria-label="AI version for photos, video and Coach" value={selectedModel} disabled={analyzing} onChange={(event) => chooseModel(event.target.value as typeof selectedModel)}>{COACH_MODEL_OPTIONS.map((option) => <option key={option.model} value={option.model}>{option.label}</option>)}</select></label>
